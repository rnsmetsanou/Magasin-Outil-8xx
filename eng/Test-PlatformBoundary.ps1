...
if (-not $IsWindows) { throw 'The process qualification requires Windows.' }
...
$start.ArgumentList.Add('--simulation')
...
try {
    ...
    do {
        $line = $hostProcess.StandardOutput.ReadLineAsync($deadline.Token).AsTask().GetAwaiter().GetResult()
